...
func Part1(line string) int {
	...
	n, _ := strconv.Atoi(fmt.Sprintf("%s%s", numbers[0], numbers[len(numbers)-1]))
...
func Part2(line string) int {
	...
	// Function to find the longest valid number at the start of the string.
	findLongestNumber := func(s string) (string, bool) {
		// Check for individual numeric digits first.
		if len(s) > 0 && s[0] >= '0' && s[0] <= '9' {
			return string(s[0]), true
		}
		// Check for spelled-out digits.
		for len(s) > 0 {
			if num, exists := digitMap[s]; exists {
				return num, true
			}
			s = s[:len(s)-1] // Remove the last character and try again.
		}
		return "", false
	}
	...
	for len(line) > 0 {
		if num, found := findLongestNumber(line); found {
	...
	if result.String() != "" {
		first, last := result.String()[:1], result.String()[len(result.String())-1:]
...
const puzzleInput = `shrzvdcghblt21
