...
func ParseInput() (map[int]Serial, map[int]string) {
	numberLocations := map[int]Serial{}
	partsLocations := map[int]string{}
	z := 0
	for _, line := range strings.Split(puzzleInput, "\n") {
		...
		for _, char := range line {
			...
			if _, err := strconv.Atoi(string(char)); err == nil {
				numberLocations[z] = Serial{Number: string(char)}
				continue
			}

			partsLocations[z] = string(char)

		}
	}

	return numberLocations, partsLocations
}

func Part1() int {
	numberLocations, partsLocations := ParseInput()

	total := 0
	for loc, _ := range partsLocations {
		// fmt.Println("loc", loc)
		serialNumber := 0
		if _, ok := numberLocations[loc-1]; ok {
			serialNumber, numberLocations = FindSerialNumber(numberLocations, loc-1)
			total += serialNumber
		}
		// right
		if _, ok := numberLocations[loc+1]; ok {
		...
		// above
		if _, ok := numberLocations[loc-RowLength]; ok {
			serialNumber, numberLocations = FindSerialNumber(numberLocations, loc-RowLength)
			total += serialNumber

			// top-right
		}

		if _, ok := numberLocations[loc-RowLength+1]; ok {
			serialNumber, numberLocations = FindSerialNumber(numberLocations, loc-RowLength+1)
			total += serialNumber
		...
		if _, ok := numberLocations[loc-1+RowLength]; ok {
			serialNumber, numberLocations = FindSerialNumber(numberLocations, loc+RowLength-1)
			total += serialNumber
...
func Part2() int {
	...
	total := 0
	for loc, part := range partsLocations {
		if part != "*" {
			continue
		}
		...
		if len(serialNumbers) == 2 {
			total = total + (serialNumbers[0] * serialNumbers[1])
		}
...
func FindSerialNumber(grid map[int]Serial, pos int) (int, map[int]Serial) {
	// fmt.Println("found:", pos)
	number := ""
	// right
	for i := 0; i <= 3; i++ {
		if n, ok := grid[pos+i]; ok && !n.Seen {
			// fmt.Println("bad")
			number = fmt.Sprintf("%s%s", number, n.Number)
			n.Seen = true
			grid[pos+i] = n
			continue
		}
	...
	for i := 1; i < 3; i++ {
		// fmt.Println(pos - i)
		if n, ok := grid[pos-i]; ok && !n.Seen {
			number = fmt.Sprintf("%s%s", n.Number, number)
			n.Seen = true
			grid[pos-i] = n
			continue
		}
	...
	serialNumber, err := strconv.Atoi(number)
	if err != nil {
		fmt.Println("pos", pos)
...
const puzzleInput = `.......358..........31.....339.....669.............598......328.....575......................447..650..............964...........692........
