...
var CardRankJokers = map[string]int{
	"A": 13,
	"K": 12,
	"Q": 11,
	"T": 10,
...
func GetHandPower(cards string) int {
	handMap := map[string]int{}
	for _, card := range cards {
		if c, ok := handMap[string(card)]; ok {
			handMap[string(card)] = c + 1
			continue
		}
...
func GetHandPowerJokersWild(cards string) int {
	handPower := GetHandPower(cards)
	numberOfJokers := strings.Count(cards, "J")
	if numberOfJokers == 0 {
		return handPower
	}
...
func Play(jokersWild bool) {
	hands := ParseInput(puzzleInput, jokersWild)
		...
		if hands[i].Power == hands[j].Power {
			for k := range hands[i].Cards {
				if hands[i].Cards[k] != hands[j].Cards[k] {
					if jokersWild {
						return CardRankJokers[string(hands[i].Cards[k])] < CardRankJokers[string(hands[j].Cards[k])]
					}
					return CardRank[string(hands[i].Cards[k])] < CardRank[string(hands[j].Cards[k])]
				}
			}
		}
	...
	totalWinnings := 0
	for rank := range hands {
		totalWinnings += ((rank + 1) * hands[rank].Bet)
	}
...
func ParseInput(input string, jokersWild bool) []Hand {
	hands := []Hand{}
	lines := strings.Split(input, "\n")
	GetHandPowerFn := GetHandPower
	if jokersWild {
		GetHandPowerFn = GetHandPowerJokersWild
	}
	for _, line := range lines {
		parts := strings.Split(line, " ")
		cards := strings.Trim(parts[0], " ")

		hands = append(hands, Hand{
			Cards: cards,
			Bet:   mustAtoi(strings.Trim(parts[1], " ")),
			Power: GetHandPowerFn(cards),
		})
	}
