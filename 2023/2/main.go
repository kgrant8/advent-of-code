...
const t1 = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green`
...
func main() {
	...
	for _, game := range strings.Split(puzzleInput, "\n") {
		id, valid := ProcessGameP1(game, MasterList)

		if valid {
			gameNumber, err := strconv.Atoi(id)
			if err != nil {
				fmt.Println("err", err)
			} else {
				part1 += gameNumber
...
func ProcessGameP1(line string, masterList map[string]int) (string, bool) {
...
func ProcessGameP2(line string) int {
	regex := regexp.MustCompile(`(\d+|red|green|blue)`)
	parts := regex.FindAllString(line, -1)
	dice := map[string]int{}
	count := 0
...
const puzzleInput = `Game 1: 2 blue, 3 red; 3 green, 3 blue, 6 red; 4 blue, 6 red; 2 green, 2 blue, 9 red; 2 red, 4 blue
